...
const DashboardTransferTable = () => {
  ...
  return (
    <div className="w-full h-full bg-primary rounded-md px-4 py-4 text-textColor-primary font-[Poppins]">
      {/* Header Row */}
        ...
        <div className="divide-y divide-border_color">
          {loading ? (
            ...
            transfers.slice(0, 8).map(t => (
              <div key={t.id} className="grid grid-cols-4 text-center text-sm py-3 font-normal">
          ...
          {Array.from({ length: emptyRows > 0 ? emptyRows : 0 }).map((_, idx) => (
            <div key={`empty-${idx}`} className="grid grid-cols-4 text-center text-sm py-3 font-normal opacity-30">
