...
export default function ProductInventoryPreview({ limit = 10, hidePageNumbers = false }) {
  ...
  useEffect(() => {
    async function fetchProducts() {
      ...
      // ✅ Count excluding Pending & Canceled
      const { count, error: countError } = await supabase
        .from("items")
        .select("id, added_items!inner(status)", { count: "exact", head: true })
        .not("added_items.status", "in", "(Pending,Canceled)");
      ...
      // ✅ Fetch excluding Pending & Canceled
      ...
      if (error) {
        ...
        console.log("✅ Filtered products (no Pending/Canceled):", data);
        setProducts(data);
      }
    ...
    <div>
      ...
      {loading ? (
        ...
        products.map((item) => (
          <div
            key={item.id}
            className="grid grid-cols-8 items-center border-b border-border_color px-3 py-3 text-sm hover:bg-btn-hover rounded transition cursor-pointer"
            onClick={() => (window.location.href = `/inventory/product/${item.sku}`)}
          >
            <span>{item.sku}</span>
            <span>{item.name}</span>
