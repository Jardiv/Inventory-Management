...
export default function ProductModal({ product, onClose, onUpdated }) {
  ...
  const exportPDF = () => {
    ...
    const tableColumn = ["Warehouse Name", "Current Stock"];
  ...
  // 📌 Export as CSV
  const exportCSV = () => {
    const headers = ["Warehouse Name", "Current Stock"];
    const rows = stockData.map(item =>
      [item.warehouse_name, item.quantity].join(",")
    );
        ...
        <div className="mt-4">
          ...
          >
            ...
            {activeTab === "stock" && (
              <>
                <div
                  className="grid grid-cols-2 font-semibold border-b pb-2 mb-3"
                  style={{ borderColor: "var(--color-border_color)" }}
                >
                ...
                {loadingStock ? (
                  ...
                  <div className="space-y-2">
                    {stockData.map((item, idx) => (
                      <div key={idx} className="grid grid-cols-2 py-2 hover:bg-gray-50/10 rounded">
                        <span>{item.warehouse_name}</span>
                        <span className="font-semibold">{item.quantity}</span>
                      </div>
                    ))}
                    <div 
                      className="grid grid-cols-2 py-2 mt-3 pt-3 font-bold border-t"
                      style={{ borderColor: "var(--color-border_color)" }}
                    >
