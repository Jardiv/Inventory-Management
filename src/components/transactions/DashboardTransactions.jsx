        ...
        const fetchTransactions = async () => {
            try {
                const response = await fetch(
                    "/api/transactions/logs?limit=5&sortBy=updated_at&sortOrder=desc"
                );
        ...
        <div className="flex flex-col gap-1 w-full py-4 px-2 bg-primary h-full rounded-lg">
            <div className="w-full z-10 flex justify-between sticky bg-primary p-4 top-0">
                ...
                <a href="/transaction/logs/" className="w-fit">
                    <button className="text-btn-primary hover:text-purple-300 text-sm sm:text-base cursor-pointer">
                        See All
                    </button>
                </a>
            </div>
            <div className={`p-2 ${loading ? 'scrollbar-hidden overflow-hidden' : 'overflow-auto '}`}>
                {loading ? (
                    <ul className="flex flex-col gap-2">
                        {[...Array(5)].map((_, i) => (
                            <DashboardTransactionSkeleton key={i} />
                        ))}
                    </ul>
                ) : (
                    <ul className="flex flex-col gap-2">
                        {transactions.map((tx) => (
                            ...
                            >
                                <div>
                                    ...
                                    <p className="text-sm text-textColor-tertiary">
