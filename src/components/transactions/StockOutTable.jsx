	...
	useEffect(() => {
		async function fetchTransactions() {
			...
			try {
				...
				const res = await fetch(`/api/transactions/transactions/?direction=out&limit=${fetchLimit}&offset=${offset}`);
							...
							{transactions.map((log) => (
								<tr key={log.id}
									...
									<td className="table-data text-center">
										<span className={`inline-block w-[6rem] px-3 py-1 text-sm font-semibold rounded-full ${statusColorMap[log.status] || 'bg-textColor-tertiary/10 text-textColor-tertiary'}`}>
											{log.status}
										</span>
									</td>
								</tr>
				...
				<div className="flex justify-between items-center pt-6 border-t border-border_color flex-shrink-0 mt-4">
					...
					{paginationData.totalItems > 0 && paginationData.totalPages > 1 && (
						<div className="flex items-center gap-1">
							...
							{paginationPages.map((page, index) => page === '...' ? <span key={index} className="px-3 py-2 text-gray-500">...</span> : <a key={page} href={`?page=${page}`} className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${page === paginationData.currentPage ? 'bg-btn-primary text-white hover:bg-btn-hover' : 'text-textColor-primary hover:bg-tbl-hover hover:text-white'}`}>{page}</a>)}
